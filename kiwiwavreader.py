...
def read_kiwiwav(f):
    ...
    if data[0:4] != b"RIFF":
        raise Exception(f"{f}: not a RIFF")
    fsize, = struct.unpack("<I", data[4:8])
    if data[8:12] != b"WAVE":
        raise Exception(f"{f}: not a WAVE")
    if data[12:16] != b"fmt ":
        raise Exception(f"{f}: no fmt chunk found")
    lfmt, fmtt, nchannels, sr, br, ba, bits = struct.unpack("<IHHIIHH", data[16:36])
    if lfmt != 16:
        raise Exception(f"{f}: invalid fmt chunk length")
    if nchannels != 2:
        raise Exception(f"{f}: expected 2 channels")
    if bits != 16:
        raise Exception(f"{f}: expected 16-bit samples")
    ...
    if len(set(len(x["data"]) for x in chunks_raw)) != 1:
        raise Exception(f"{f}: unsupported: chunks have different lengths")

    dt_ns_chunks = [] 
    gpsns_prev = 0
    for i, chunk in enumerate(chunks_raw):
        if i == 0 and chunk["gpsns"] != 0:
            raise Exception(f"{f}: first chunk has nonzero timestamp")
    ...
    if len(dt_ns_chunks) == 0:
        raise Exception(f"{f}: no usable timestamps")
    if len(dt_ns_chunks) + 2 != len(chunks_raw):
        raise Exception(f"{f}: dt_ns_chunks length doesn't fit together with chunks_raw length")
    ...
    for i, chunk in enumerate(chunks_raw):
        ...
        if dt != 0:
            raise Exception(f"{f}: chunks {i} and {i+1} are spaced weirdly")
    ...
    if len(samples_all) != len(timestamps_all):
        raise Exception(f"{f}: len(samples_all) != len(timestamps_all)")
